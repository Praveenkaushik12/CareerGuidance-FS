...
export default function AskCounsellor() {
  ...
  const generateStars = (rating) => {
    const stars = [];
    for (let i = 1; i <= 5; i++) {
      const starClass = i <= rating ? AskCounsellorCSS.starsFilled : AskCounsellorCSS.starEmpty;
      stars.push(
  ...
  const handleChatButtonClick = async (counsellor) => {
    if (is_exist) {
      try {
        await createChannel(counsellor.id, counsellor.name, "", navigate);
      } catch (error) {
    ...
    <main role="main" className={AskCounsellorCSS.MainDiv}>
      <div className="text-center m-3">
        <h1 className={AskCounsellorCSS.CounsellorsHeading}>
          CareerGuidance Top Counsellors
        </h1>

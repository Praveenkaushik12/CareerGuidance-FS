...
export default function CounsellorsReport() {
  ...
  useEffect(() => { dispatch(getApprovedCounsellors()); }, [dispatch]);
  ...
  const selectedCounsellor = rows?.find(r => r.id === selectedRow);

  return (
    <div style={s.page}>
      <div style={s.banner}>
        <div>
          <div style={s.bannerTitle}><i className="fas fa-user-tie" style={{ marginRight: 10 }} />Counsellors Report</div>
          <div style={s.bannerSub}>{filtered.length} approved counsellor{filtered.length !== 1 ? 's' : ''}</div>
        </div>
      </div>
      ...
      <div style={s.toolbar}>
        ...
        <div style={s.countBadge}>{filtered.length} counsellors</div>
      </div>
      ...
      <div style={s.tableWrap}>
        <table style={s.table}>
          ...
          <tbody>
            {paged.length === 0 ? (
              <tr><td colSpan={4} style={s.empty}>No counsellors found</td></tr>
            ) : paged.map((row, i) => (
              <tr key={row.id} style={s.tr}>
                <td style={{ ...s.td, color: '#aaa', fontSize: 12 }}>{page * PAGE_SIZE + i + 1}</td>
                <td style={s.td}>
                  <div style={s.nameCell}>
                    <div style={{ ...s.avatar, background: 'linear-gradient(135deg,#7c3aed,#4a148c)' }}>
                      {(row.name || row.email || 'C').charAt(0).toUpperCase()}
                    </div>
      ...
      <Modal open={confirmOpen} onClose={() => setConfirm(false)}>
        <div style={s.modalHeader}>
          <span style={s.modalTitle}>Delete Counsellor</span>
        ...
        <div style={s.modalBody}>
          <div style={{ textAlign: 'center', padding: '8px 0 16px' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 6 }}>Delete this counsellor?</div>
            {selectedCounsellor && <div style={{ color: '#888', fontSize: 14 }}>{selectedCounsellor.name || selectedCounsellor.email}</div>}
            <div style={{ color: '#aaa', fontSize: 12, marginTop: 4 }}>This will permanently remove their account and all associated data.</div>
          </div>
        </div>
